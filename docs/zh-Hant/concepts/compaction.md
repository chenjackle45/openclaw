...
請參閱 [壓縮設定和模式](/zh-Hant/concepts/compaction) 了解 `agents.defaults.compaction` 設定。
...
在壓縮之前，OpenClaw 可以運行**靜默記憶體刷新**輪次，將持久備註儲存到磁碟。請參閱 [記憶體](/zh-Hant/concepts/memory) 了解詳情和設定。
