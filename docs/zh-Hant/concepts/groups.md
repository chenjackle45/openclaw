...
- 設定鍵和預設值：[Gateway 設定](/zh-Hant/gateway/configuration#agentsdefaultssandbox)
- 除錯為什麼工具被阻止：[沙盒 vs 工具策略 vs 提升](/zh-Hant/gateway/sandbox-vs-tool-policy-vs-elevated)
- 綁定掛載詳情：[沙盒](/zh-Hant/gateway/sandboxing#custom-bind-mounts)
...
請參閱 [群組訊息](/zh-Hant/concepts/group-messages) 了解僅 WhatsApp 行為（歷史注入、提及處理詳情）。
