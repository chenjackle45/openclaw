...
> **剛接觸組態設定？** 查看 [Configuration Examples](/zh-Hant/gateway/configuration-examples) 指南以獲取包含詳細解釋的完整範例！
...
參閱 [/environment](/zh-Hant/environment) 以獲取完整優先順序與來源。
...
亦參閱: [/concepts/oauth](/zh-Hant/concepts/oauth)
...
參閱 [Multi-Agent Sandbox & Tools](/zh-Hant/multi-agent-sandbox-tools) 以獲取優先順序與其他範例。
...
- `/reasoning stream` 將 Reasoning 串流至 Draft，然後發送最終答案。
  Retry Policy 預設值與行為記錄在 [Retry policy](/zh-Hant/concepts/retry)。
...
參見 [Messages](/zh-Hant/concepts/messages) 以了解 Queueing, Sessions, 與 Streaming Context。
...
- **Soft-trim**: 僅針對 _Oversized_ Tool Results。保留開頭 + 結尾並在中間插入 `...`。
...
參閱 [/concepts/session-pruning](/zh-Hant/concepts/session-pruning) 以了解行為細節。

#### `agents.defaults.compaction` (reserve headroom + memory flush)

`agents.defaults.compaction.mode` 選擇 Compaction Summarization Strategy。預設為 `default`；設定 `safeguard` 以啟用針對極長 Histories 的 Chunked Summarization。參閱 [/concepts/compaction](/zh-Hant/concepts/compaction)。
...
- `agents.defaults.humanDelay`: 第一個 Block Reply 之後的隨機暫停。
  ...
  ```
  參閱 [/concepts/streaming](/zh-Hant/concepts/streaming) 以了解行為 + Chunking 細節。
...
- `session.typingIntervalSeconds`: Per-session Refresh Interval Override.
  參閱 [/concepts/typing-indicators](/zh-Hant/concepts/typing-indicators) 以了解行為細節。
...
詳情：[Sandboxing](/zh-Hant/gateway/sandboxing)

Defaults (If enabled):

- scope: `"agent"` (One container + workspace per agent) -基於 Debian bookworm-slim 的 Image
...
- Tool Policy: 僅允許 `exec`, `process`, `read`, `write`, `edit`, `apply_patch`, `sessions_list`, `sessions_history`, `sessions_send`, `sessions_spawn`, `session_status` (Deny Wins)
  ...
  - Sandbox Policy 中支援 Tool Group Shorthands: `group:runtime`, `group:fs`, `group:sessions`, `group:memory` (參見 [Sandbox vs Tool Policy vs Elevated](/zh-Hant/gateway/sandbox-vs-tool-policy-vs-elevated#tool-groups-shorthands))
