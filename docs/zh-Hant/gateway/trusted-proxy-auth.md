---
summary: "將 Gateway 認證委託給受信任的反向代理（Pomerium、Caddy、nginx + OAuth）"
read_when:
  - 在身份感知代理後執行 OpenClaw
  - 在 OpenClaw 前設定 Pomerium、Caddy 或 nginx 與 OAuth
  - 修正反向代理設定的 WebSocket 1008 未授權錯誤
  - 決定在哪裡設定 HSTS 和其他 HTTP 強化標頭
title: "Trusted Proxy Auth（受信任代理認證）"
---
...
- [Security](/zh-Hant/gateway/security) — full security guide
- [Configuration](/zh-Hant/gateway/configuration) — config reference
- [Remote Access](/zh-Hant/gateway/remote) — other remote access patterns
- [Tailscale](/zh-Hant/gateway/tailscale) — simpler alternative for tailnet-only access
