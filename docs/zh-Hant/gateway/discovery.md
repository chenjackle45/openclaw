---
title: "Discovery and Transports（發現與傳輸）"
...
- **Legacy TCP bridge (deprecated/removed)**: 舊版 Node 傳輸 (參閱 [Bridge protocol](/zh-Hant/gateway/bridge-protocol))；不再廣播以供發現。
...
故障排除與 Beacon 細節: [Bonjour](/zh-Hant/gateway/bonjour)。
...
參閱 [Remote access](/zh-Hant/gateway/remote)。
...
- 配對請求在 Gateway 中建立/核准/拒絕 (參閱 [Gateway pairing](/zh-Hant/gateway/pairing))。
- Gateway 強制執行:
  - Auth (Token / Keypair)
