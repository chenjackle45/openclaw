...
本地運行是可行的，但 OpenClaw 預期有大 Context + 對 Prompt Injection 的強大防禦。小型 Cards 會截斷 Context 並洩漏安全性。目標定高一點：**≥2 台 maxed-out Mac Studios 或同等級的 GPU Rig (~$30k+)**。單張 **24 GB** GPU 僅適用於較輕的 Prompts 且延遲較高。請使用 **您能運行的最大 / 全尺寸模型變體**；過度量化或“小型” Checkpoints 會增加 Prompt-injection 風險 (參閱 [Security](/zh-Hant/gateway/security))。
