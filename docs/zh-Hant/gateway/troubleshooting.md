...
若您只想快速分類，請從 FAQ 的 [First 60 seconds](/zh-Hant/help/faq#first-60-seconds-if-somethings-broken) 開始。本頁面將更深入探討執行時期失敗與診斷。

供應商專屬捷徑: [/channels/troubleshooting](/zh-Hant/channels/troubleshooting)
...
參閱: [Health checks](/zh-Hant/gateway/health) 與 [Logging](/zh-Hant/logging)。
...
更多細節: [Anthropic](/zh-Hant/providers/anthropic) 與 [OAuth](/zh-Hant/concepts/oauth)。
...
- 偏好透過 [Tailscale Serve](/zh-Hant/gateway/tailscale) 使用 HTTPS。
- 或在 Gateway Host 本地開啟: `http://127.0.0.1:18789/`。
- 若必須停留在 HTTP，啟用 `gateway.controlUi.allowInsecureAuth: true` 並使用 Gateway Token (Token-only; 無 Device Identity/Pairing)。參閱 [Control UI](/zh-Hant/web/control-ui#insecure-http)。

### CI Secrets Scan Failed

這表示 `detect-secrets` 發現了不在 Baseline 中的新候選者。
遵循 [Secret scanning](/zh-Hant/gateway/security#secret-scanning-detect-secrets)。
...
參閱 [/logging](/zh-Hant/logging) 取得完整格式、設定與存取概覽。
...
在 `host=gateway` 上運行的 Exec 會將您的 Login-shell `PATH` 合併入 Exec Environment，因此遺失工具通常表示您的 Shell Init 未匯出它們 (或設定 `tools.exec.pathPrepend`)。參閱 [/tools/exec](/zh-Hant/tools/exec)。
...
**修復:** 保持單一活躍 Workspace 並封存/移除其餘。參閱 [Agent workspace](/zh-Hant/concepts/agent-workspace#extra-workspace-folders)。
...
參閱: [Models CLI](/zh-Hant/cli/models) 與 [Model providers](/zh-Hant/concepts/model-providers)。
...
參閱 [WhatsApp setup](/zh-Hant/channels/whatsapp)。
...
參閱 [Streaming](/zh-Hant/concepts/streaming)。
...
文件: [Discord](/zh-Hant/channels/discord), [Channels troubleshooting](/zh-Hant/channels/troubleshooting)。
...
參閱 [Tools](/zh-Hant/tools) 與 [TypeBox schemas](/zh-Hant/concepts/typebox)。
...
_"Have you tried turning it off and on again?"_ — 每個 IT 人員說過的話
...
**完整指南:** 參閱 [browser-linux-troubleshooting](/zh-Hant/tools/browser-linux-troubleshooting)
