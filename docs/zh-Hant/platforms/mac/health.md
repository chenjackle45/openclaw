...
- 您仍可使用 [Gateway health](/zh-Hant/gateway/health) 中的 CLI 流程 (`openclaw status`, `openclaw status --deep`, `openclaw health --json`)，並 tail `/tmp/openclaw/openclaw-*.log` 查看 `web-heartbeat` / `web-reconnect` 相關日誌。
