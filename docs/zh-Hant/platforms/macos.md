...
- **IP 回報：** SSH 通道使用 loopback，因此 Gateway 會看到節點 IP 為 `127.0.0.1`。若希望顯示真實客戶端 IP，請使用 **Direct (ws/wss)** 傳輸（參閱 [macOS remote access](/zh-Hant/platforms/mac/remote)）。

設定步驟請參閱 [macOS remote access](/zh-Hant/platforms/mac/remote)。
協定詳情請參閱 [Gateway protocol](/zh-Hant/gateway/protocol)。
