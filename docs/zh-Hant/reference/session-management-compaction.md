...
title: "Session Management Deep Dive（Session 管理深入探討）"
---
...
- See [Memory](/zh-Hant/concepts/memory) for the workspace file layout and write patterns.
...
- Session key wrong? Start with [/concepts/session](/zh-Hant/concepts/session) and confirm the `sessionKey` in `/status`.
