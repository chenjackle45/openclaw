---
summary: "子代理：生成隔離的代理執行並將結果回報至請求者聊天室"
read_when:
...
- For ACP harness sessions (Codex, Claude Code, Gemini CLI), use `sessions_spawn` with `runtime: "acp"` and see [ACP Agents](/zh-Hant/tools/acp-agents).
...
See [Configuration Reference](/zh-Hant/gateway/configuration-reference) and [Slash commands](/zh-Hant/tools/slash-commands) for current adapter details.
