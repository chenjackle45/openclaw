...
是的，透過 `bindings` 將每個發送者路由到不同的代理（peer `kind: "dm"`，sender E.164 如 `+15551234567`）。回覆仍然來自**同一個 WhatsApp 帳戶**，直接聊天會歸納到每個代理的主會話，因此使用**每人一個代理**。私訊存取控制（`dmPolicy`/`allowFrom`）是每個 WhatsApp 帳戶全域的。請參閱 [多代理路由](/zh-Hant/concepts/multi-agent)。
...
- 疑難排解指南：[Gateway 疑難排解](/zh-Hant/gateway/troubleshooting)。
