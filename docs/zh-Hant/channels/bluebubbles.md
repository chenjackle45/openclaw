...
- `channels.bluebubbles.groupAllowFrom` 控制誰可以在設定 `allowlist` 時於群組中觸發。
...
- `channels.bluebubbles.allowFrom`：DM 允許清單（handle、電子郵件、E.164 號碼、`chat_id:*`、`chat_guid:*`）。
- `channels.bluebubbles.groupPolicy`：`open | allowlist | disabled`（預設：`allowlist`）。
...
- 直接 handle：`+15555550123`、`user@example.com`
  - 如果直接 handle 沒有現有的 DM 聊天，OpenClaw 會透過 `POST /api/v1/chat/new` 建立一個。這需要啟用 BlueBubbles Private API。
